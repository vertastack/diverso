...
const HeroSlider: React.FC = () => {
  ...
  useEffect(() => {
    const fetchHeroData = async () => {
      try {
        ...
        const response = await strapiService.getContent<HeroData>(
          "diverso/hero",
          {
            populate: {
              slides: {
  ...
  useEffect(() => {
    if (splideRef.current && heroData?.slides?.length) {
  ...
  if (!heroData) {
    return (
      <section className="relative h-[600px] lg:h-[700px] bg-gray-200 flex items-center justify-center">
        ...
        <div className="container mx-auto px-4">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4 text-white">
            <p className="text-lg md:text-xl font-medium text-center md:text-left">
              {heroData.bottomBarText}
            </p>
            <div className="flex items-center gap-3 shrink-0">
              <span className="text-lg font-medium">{heroData.phoneText}</span>
