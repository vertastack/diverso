...
async function getMaintenancePlanData(): Promise<MaintenancePlanData | null> {
  try {
    ...
    console.error("Error fetching maintenance plan data:", error);
    return null;
  }
}
...
const MaintenancePlanSlider = ({
  images,
}: {
  images: MaintenancePlanData["galleryImages"];
}) => {
...
export default async function MaintenancePlanPage() {
  ...
  return (
    <>
      ...
      <section className="bg-primary py-8">
        <div className="container mx-auto px-4">
          <p className="text-white text-center text-lg md:text-xl font-medium italic">
            {maintenancePlanData.headerQuote}
          </p>
        </div>
      </section>
            ...
            <div className="lg:col-span-1">
              <div className="bg-secondary p-8 text-white sticky top-8">
                <ul className="space-y-4">
                  {maintenancePlanData.features.map((feature, index) => (
                    <li
                      key={feature.id}
                      className={`flex items-start gap-3 text-white ${
                        index < maintenancePlanData.features.length - 1
      ...
      <MaintenancePlanSlider images={maintenancePlanData.galleryImages} />

      {/* Info Section */}
      <section className="py-16 bg-neutral-gray-light">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl md:text-4xl font-bold text-neutral-dark mb-6">
            {maintenancePlanData.infoSectionTitle}
          </h2>
          <div className="space-y-4 text-neutral-charcoal leading-relaxed max-w-5xl">
            {maintenancePlanData.infoSectionContent
              .split("\n\n")
              .map((paragraph, index) => (
                <p key={index}>{paragraph}</p>
              ))}
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-6 text-white">
            <div>
              <p className="text-lg md:text-xl font-medium mb-2">
                {maintenancePlanData.ctaTitle}
              </p>
              <p className="text-lg">{maintenancePlanData.ctaSubtitle}</p>
            </div>
            <div className="flex flex-col sm:flex-row items-center gap-4 shrink-0">
              <div className="flex items-center gap-2 bg-white text-primary px-5 py-3 border-2 border-dashed border-white">
                <Phone size={20} />
                <a
                  href={`tel:${maintenancePlanData.phoneNumber.replace(/\s/g, "")}`}
                  className="font-bold text-lg hover:underline"
                >
                  {maintenancePlanData.phoneNumber}
                </a>
              </div>
      ...
      <section className="py-16 bg-neutral-gray-light">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
            {maintenancePlanData.processSteps.map((step, index) => {
              ...
              return (
                <div key={step.id} className="relative">
                  ...
                  >
                    ...
                    {index < maintenancePlanData.processSteps.length - 1 && (
            ...
            <div className="flex flex-col lg:flex-row gap-8">
              <div className="lg:w-2/3">
                <h2 className="text-3xl md:text-4xl font-bold text-neutral-dark mb-4">
                  {maintenancePlanData.formTitle}
                </h2>
                <p className="text-neutral-charcoal mb-8">
                  {maintenancePlanData.formSubtitle}
                </p>
          ...
          <div className="bg-success text-white p-4 mt-8 flex items-start gap-3">
            ...
            <p className="text-sm">
              <strong>{maintenancePlanData.noticeText}</strong>
            </p>
          </div>
        </div>
      </section>
