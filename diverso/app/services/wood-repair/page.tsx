...
async function getWoodRepairData(): Promise<WoodRepairData | null> {
  try {
    const strapiService = new StrapiService();
    const response = await strapiService.getContent<WoodRepairData>(
      "diverso-wood-repair-services",
      {
        populate: {
          seo: {
            populate: {
    ...
    console.error("Error fetching wood repair data:", error);
    return null;
  }
}
...
const WoodRepairSlider = ({
  images,
}: {
  images: WoodRepairData["galleryImages"];
}) => {
...
export default async function WoodRepairPage() {
  const woodRepairData = await getWoodRepairData();
            ...
            <div className="lg:col-span-1">
              <div className="bg-secondary p-8 text-white sticky top-8">
                <ul className="space-y-4">
                  {woodRepairData.features.map((feature, index) => (
                    <li
                      key={feature.id}
                      className={`flex items-start gap-3 text-white ${
                        index < woodRepairData.features.length - 1
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-6 text-white">
            <div>
              <p className="text-lg md:text-xl font-medium mb-2">
                {woodRepairData.ctaTitle}
              </p>
              <p className="text-lg">{woodRepairData.ctaSubtitle}</p>
            </div>
            <div className="flex flex-col sm:flex-row items-center gap-4 shrink-0">
              <div className="flex items-center gap-2 bg-white text-primary px-5 py-3 border-2 border-dashed border-white">
                <Phone size={20} />
                <a
                  href={`tel:${woodRepairData.phoneNumber.replace(/\s/g, "")}`}
                  className="font-bold text-lg hover:underline"
                >
                  {woodRepairData.phoneNumber}
                </a>
              </div>
      ...
      <section className="py-16 bg-neutral-gray-light">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
            {woodRepairData.processSteps.map((step, index) => {
              ...
              return (
                <div key={step.id} className="relative">
                  ...
                  >
                    ...
                    {index < woodRepairData.processSteps.length - 1 && (
            ...
            <div className="flex flex-col lg:flex-row gap-8">
              <div className="lg:w-2/3">
                <h2 className="text-3xl md:text-4xl font-bold text-neutral-dark mb-4">
                  {woodRepairData.formTitle}
                </h2>
                <p className="text-neutral-charcoal mb-8">
                  {woodRepairData.formSubtitle}
                </p>
          ...
          <div className="bg-success text-white p-4 mt-8 flex items-start gap-3">
            ...
            <p className="text-sm">
              <strong>{woodRepairData.noticeText}</strong>
            </p>
          </div>
        </div>
      </section>
