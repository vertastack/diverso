...
interface DecoratiefStucwerkData {
  id: number;
  seo: SEOData;
  pageTitle: string;
...
async function getDecoratiefStucwerkData(): Promise<DecoratiefStucwerkData | null> {
  try {
    const strapiService = new StrapiService();
    const response = await strapiService.getContent<DecoratiefStucwerkData>(
      "diverso-decoratief-stucwerk-service",
      {
        populate: {
          seo: {
            populate: {
    ...
    console.error("Error fetching decoratief stucwerk data:", error);
    return null;
  }
}
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  if (decoratiefStucwerkData && decoratiefStucwerkData.seo) {
    return {
      title: decoratiefStucwerkData.seo.metaTitle,
      description: decoratiefStucwerkData.seo.metaDescription,
    };
  }

  // Fallback metadata
  return {
    title: "Decoratief Stucwerk | Diverso",
    description:
      "Professioneel decoratief stucwerk voor uw huis of bedrijf. Ervaren stukadoors met meer dan 20 jaar ervaring in Zutphen en omgeving.",
  };
}

// Image Slider Component
const DecoratiefStucwerkSlider = ({
  images,
}: {
  images: DecoratiefStucwerkData["galleryImages"];
}) => {
        ...
        {images.map((imageData) => (
          <div key={imageData.id} className="relative h-64 bg-gray-200">
            <img
              ...
              alt={getImageAlt(imageData.image, "Decoratief Stucwerk")}
...
export default async function DecoratiefStucwerkPage() {
  ...
  const seo = decoratiefStucwerkData.seo
    ? {
        metaTitle: decoratiefStucwerkData.seo.metaTitle,
        metaDescription: decoratiefStucwerkData.seo.metaDescription,
        metaImage: decoratiefStucwerkData.seo.metaImage || { url: "" },
        metaKeywords: decoratiefStucwerkData.seo.metaKeywords || "",
        metaRobots: decoratiefStucwerkData.seo.metaRobots || "index, follow",
        metaUrl: decoratiefStucwerkData.seo.metaUrl || "https://diverso.com",
      }
    : {
        metaTitle: "Decoratief Stucwerk | Diverso",
        metaDescription: "Professionele diensten voor decoratief stucwerk.",
        metaImage: { url: "" },
        metaKeywords: "",
  ...
  return (
    <>
      ...
      <section className="bg-primary py-8">
        <div className="container mx-auto px-4">
          <p className="text-white text-center text-lg md:text-xl font-medium italic">
            {decoratiefStucwerkData.headerQuote}
          </p>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div className="lg:col-span-2">
              <h1 className="text-3xl md:text-4xl font-bold text-neutral-dark mb-8">
                {decoratiefStucwerkData.mainTitle}
              </h1>

              <div className="space-y-6 text-neutral-charcoal leading-relaxed">
                {decoratiefStucwerkData.mainContent
                  .split("\n\n")
                  .map((paragraph, index) => (
      ...
      <DecoratiefStucwerkSlider images={decoratiefStucwerkData.galleryImages} />

      {/* Info Section */}
      <section className="py-16 bg-neutral-gray-light">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl md:text-4xl font-bold text-neutral-dark mb-6">
            {decoratiefStucwerkData.infoSectionTitle}
          </h2>
          <div className="space-y-4 text-neutral-charcoal leading-relaxed max-w-5xl">
            {decoratiefStucwerkData.infoSectionContent
              .split("\n\n")
              .map((paragraph, index) => (
                <p key={index}>{paragraph}</p>
              ))}
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-6 text-white">
            <div>
              <p className="text-lg md:text-xl font-medium mb-2">
                {decoratiefStucwerkData.ctaTitle}
              </p>
              <p className="text-lg">{decoratiefStucwerkData.ctaSubtitle}</p>
            </div>
            <div className="flex flex-col sm:flex-row items-center gap-4 shrink-0">
              <div className="flex items-center gap-2 bg-white text-primary px-5 py-3 border-2 border-dashed border-white">
                <Phone size={20} />
                <a
                  href={`tel:${decoratiefStucwerkData.phoneNumber.replace(
                    /\s/g,
                    "",
                  )}`}
                  className="font-bold text-lg hover:underline"
                >
                  {decoratiefStucwerkData.phoneNumber}
                </a>
              </div>
      ...
      <section className="py-16 bg-neutral-gray-light">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
            {decoratiefStucwerkData.processSteps.map((step, index) => {
            ...
            <div className="flex flex-col lg:flex-row gap-8">
              <div className="lg:w-2/3">
                <h2 className="text-3xl md:text-4xl font-bold text-neutral-dark mb-4">
                  {decoratiefStucwerkData.formTitle}
                </h2>
                <p className="text-neutral-charcoal mb-8">
                  {decoratiefStucwerkData.formSubtitle}
                </p>
          ...
          <div className="bg-success text-white p-4 mt-8 flex items-start gap-3">
            ...
            <p className="text-sm">
              <strong>{decoratiefStucwerkData.noticeText}</strong>
            </p>
          </div>
        </div>
      </section>
