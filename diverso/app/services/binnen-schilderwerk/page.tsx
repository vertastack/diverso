...
interface BinnenSchilderwerkData {
  id: number;
  seo: SEOData;
  pageTitle: string;
...
async function getBinnenSchilderwerkData(): Promise<BinnenSchilderwerkData | null> {
  try {
    const strapiService = new StrapiService();
    const response = await strapiService.getContent<BinnenSchilderwerkData>(
      "diverso-binnen-schilderwerk-service",
      {
        populate: {
          seo: {
            populate: {
    ...
    console.error("Error fetching binnen schilderwerk data:", error);
    return null;
  }
}

export async function generateMetadata(): Promise<Metadata> {
  const binnenSchilderwerkData = await getBinnenSchilderwerkData();

  if (binnenSchilderwerkData && binnenSchilderwerkData.seo) {
    return {
      title: binnenSchilderwerkData.seo.metaTitle,
      description: binnenSchilderwerkData.seo.metaDescription,
    };
  }

  // Fallback metadata
  return {
    title: "Binnen Schilderwerk | Diverso",
    description:
      "Professioneel binnenschilderwerk voor uw huis of bedrijf. Ervaren schilders met meer dan 20 jaar ervaring in Zutphen en omgeving.",
  };
}

// Image Slider Component
const BinnenSchilderwerkSlider = ({
  images,
}: {
  images: BinnenSchilderwerkData["galleryImages"];
}) => {
        ...
        {images.map((imageData) => (
          <div key={imageData.id} className="relative h-64 bg-gray-200">
            <img
              ...
              alt={getImageAlt(imageData.image, "Binnen schilderwerk")}
...
export default async function BinnenSchilderwerkPage() {
  const binnenSchilderwerkData = await getBinnenSchilderwerkData();

  if (!binnenSchilderwerkData) {
    return (
      <>
        <Header />
  ...
  const seo = binnenSchilderwerkData.seo
    ? {
        metaTitle: binnenSchilderwerkData.seo.metaTitle,
        metaDescription: binnenSchilderwerkData.seo.metaDescription,
        metaImage: binnenSchilderwerkData.seo.metaImage || { url: "" },
        metaKeywords: binnenSchilderwerkData.seo.metaKeywords || "",
        metaRobots: binnenSchilderwerkData.seo.metaRobots || "index, follow",
        metaUrl: binnenSchilderwerkData.seo.metaUrl || "https://diverso.com",
      }
    : {
        metaTitle: "Binnen Schilderwerk | Diverso",
        metaDescription: "Professionele diensten voor binnenschilderwerk.",
        metaImage: { url: "" },
        metaKeywords: "",
  ...
  return (
    <>
      ...
      <section className="bg-primary py-8">
        <div className="container mx-auto px-4">
          <p className="text-white text-center text-lg md:text-xl font-medium italic">
            {binnenSchilderwerkData.headerQuote}
          </p>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div className="lg:col-span-2">
              <h1 className="text-3xl md:text-4xl font-bold text-neutral-dark mb-8">
                {binnenSchilderwerkData.mainTitle}
              </h1>

              <div className="space-y-6 text-neutral-charcoal leading-relaxed">
                {binnenSchilderwerkData.mainContent
                  .split("\n\n")
                  .map((paragraph, index) => (
            ...
            <div className="lg:col-span-1">
              <div className="bg-secondary p-8 text-white sticky top-8">
                <ul className="space-y-4">
                  {binnenSchilderwerkData.features.map((feature, index) => (
                    <li
                      key={feature.id}
                      className={`flex items-start gap-3 text-white ${
                        index < binnenSchilderwerkData.features.length - 1
      ...
      {/* BinnenSchilderwerk Images Slider */}
      <BinnenSchilderwerkSlider images={binnenSchilderwerkData.galleryImages} />

      {/* Info Section */}
      <section className="py-16 bg-neutral-gray-light">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl md:text-4xl font-bold text-neutral-dark mb-6">
            {binnenSchilderwerkData.infoSectionTitle}
          </h2>
          <div className="space-y-4 text-neutral-charcoal leading-relaxed max-w-5xl">
            {binnenSchilderwerkData.infoSectionContent
              .split("\n\n")
              .map((paragraph, index) => (
                <p key={index}>{paragraph}</p>
              ))}
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-6 text-white">
            <div>
              <p className="text-lg md:text-xl font-medium mb-2">
                {binnenSchilderwerkData.ctaTitle}
              </p>
              <p className="text-lg">{binnenSchilderwerkData.ctaSubtitle}</p>
            </div>
            <div className="flex flex-col sm:flex-row items-center gap-4 shrink-0">
              <div className="flex items-center gap-2 bg-white text-primary px-5 py-3 border-2 border-dashed border-white">
                <Phone size={20} />
                <a
                  href={`tel:${binnenSchilderwerkData.phoneNumber.replace(/\s/g, "")}`}
                  className="font-bold text-lg hover:underline"
                >
                  {binnenSchilderwerkData.phoneNumber}
                </a>
              </div>
      ...
      <section className="py-16 bg-neutral-gray-light">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
            {binnenSchilderwerkData.processSteps.map((step, index) => {
              ...
              return (
                <div key={step.id} className="relative">
                  ...
                  >
                    ...
                    {index < binnenSchilderwerkData.processSteps.length - 1 && (
            ...
            <div className="flex flex-col lg:flex-row gap-8">
              <div className="lg:w-2/3">
                <h2 className="text-3xl md:text-4xl font-bold text-neutral-dark mb-4">
                  {binnenSchilderwerkData.formTitle}
                </h2>
                <p className="text-neutral-charcoal mb-8">
                  {binnenSchilderwerkData.formSubtitle}
                </p>
          ...
          <div className="bg-success text-white p-4 mt-8 flex items-start gap-3">
            ...
            <p className="text-sm">
              <strong>{binnenSchilderwerkData.noticeText}</strong>
            </p>
          </div>
        </div>
      </section>
