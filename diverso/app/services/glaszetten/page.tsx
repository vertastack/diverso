...
async function getGlaszettenData(): Promise<GlaszettenData | null> {
  try {
    const strapiService = new StrapiService();
    const response = await strapiService.getContent<GlaszettenData>(
      "diverso-glaszetten-service",
      {
        populate: {
          seo: {
            populate: {
    ...
    console.error("Error fetching glaszetten data:", error);
    return null;
  }
}
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  if (glaszettenData && glaszettenData.seo) {
    return {
      title: glaszettenData.seo.metaTitle,
      description: glaszettenData.seo.metaDescription,
    };
  }

  // Fallback metadata
  return {
    title: "Glaszetten | Diverso",
    description:
      "Professioneel glaszetten voor uw huis of bedrijf. Ervaren vakmensen met meer dan 20 jaar ervaring in Zutphen en omgeving.",
  };
}

// Image Slider Component
const GlaszettenSlider = ({
  images,
}: {
  images: GlaszettenData["galleryImages"];
}) => {
        ...
        {images.map((imageData) => (
          <div key={imageData.id} className="relative h-64 bg-gray-200">
            <img
              ...
              alt={getImageAlt(imageData.image, "Glaszetten")}
...
export default async function GlaszettenPage() {
  ...
  const seo = glaszettenData.seo
    ? {
        metaTitle: glaszettenData.seo.metaTitle,
        metaDescription: glaszettenData.seo.metaDescription,
        metaImage: glaszettenData.seo.metaImage || { url: "" },
        metaKeywords: glaszettenData.seo.metaKeywords || "",
        metaRobots: glaszettenData.seo.metaRobots || "index, follow",
        metaUrl: glaszettenData.seo.metaUrl || "https://diverso.com",
      }
    : {
        metaTitle: "Glaszetten | Diverso",
        metaDescription: "Professionele diensten voor glaszetten.",
        metaImage: { url: "" },
        metaKeywords: "",
            ...
            <div className="lg:col-span-1">
              <div className="bg-secondary p-8 text-white sticky top-8">
                <ul className="space-y-4">
                  {glaszettenData.features.map((feature, index) => (
                    <li
                      key={feature.id}
                      className={`flex items-start gap-3 text-white ${
                        index < glaszettenData.features.length - 1
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-6 text-white">
            <div>
              <p className="text-lg md:text-xl font-medium mb-2">
                {glaszettenData.ctaTitle}
              </p>
              <p className="text-lg">{glaszettenData.ctaSubtitle}</p>
            </div>
            <div className="flex flex-col sm:flex-row items-center gap-4 shrink-0">
              <div className="flex items-center gap-2 bg-white text-primary px-5 py-3 border-2 border-dashed border-white">
                <Phone size={20} />
                <a
                  href={`tel:${glaszettenData.phoneNumber.replace(/\s/g, "")}`}
                  className="font-bold text-lg hover:underline"
                >
                  {glaszettenData.phoneNumber}
                </a>
              </div>
      ...
      <section className="py-16 bg-neutral-gray-light">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
            {glaszettenData.processSteps.map((step, index) => {
              ...
              return (
                <div key={step.id} className="relative">
                  ...
                  >
                    ...
                    {index < glaszettenData.processSteps.length - 1 && (
            ...
            <div className="flex flex-col lg:flex-row gap-8">
              <div className="lg:w-2/3">
                <h2 className="text-3xl md:text-4xl font-bold text-neutral-dark mb-4">
                  {glaszettenData.formTitle}
                </h2>
                <p className="text-neutral-charcoal mb-8">
                  {glaszettenData.formSubtitle}
                </p>
          ...
          <div className="bg-success text-white p-4 mt-8 flex items-start gap-3">
            ...
            <p className="text-sm">
              <strong>{glaszettenData.noticeText}</strong>
            </p>
          </div>
        </div>
      </section>
